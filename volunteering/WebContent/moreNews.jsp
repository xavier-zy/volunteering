...
<body>
  ...
  <div class="container">
    ...
    <table class="table table-hover">
                     ...
                     %>
                     <tr>
                     <td><a href="news.jsp?id=<%=news.get((index-1)*10+i).getId()%>"><%=news.get((index-1)*10+i).getTitle() %></a></td>
                     <td><a href="news.jsp?id=<%=news.get((index-1)*10+i).getId()%>"><%=news.get((index-1)*10+i).getPublishtime() %></td>
                     </tr>
                     <%
                    	 }
                     }else{
					...
					%>
					 <tr>
                     <td><a href="news.jsp?id=<%=news.get((index-1)*10+i).getId()%>"><%=news.get((index-1)*10+i).getTitle() %></td>
                     <td><a href="news.jsp?id=<%=news.get((index-1)*10+i).getId()%>"><%=news.get((index-1)*10+i).getPublishtime() %></td>
                     </tr>
                     <%
							}
			...
			<span><a href="moreNews.jsp?index=1"></a></span>
			<%
			if(index!=1){
			%>
			<span><a href="moreNews.jsp?index=<%=index-1 %>"><</a></span>
			<%
			} 
			%>
			<%
			if(index-2>=1){
			%>
			<span><a href="moreNews.jsp?index=<%=index-2 %>"><%=index-2 %></a></span>
			<%
			} 
			%>
			<%
			if(index-1>=1){
			%>
			<span><a href="moreNews.jsp?index=<%=index-1 %>"><%=index-1 %></a></span>
			<%
			} 
			%>
			<span><%=index %></span>
			...
			%>
			<span><a href="moreNews.jsp?index=<%=(int)lastPage %>">>></a></span>
			</div>
		</div>
	</section>
