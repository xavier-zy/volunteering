...
public class ActivityUpdateServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String m_titlt = request.getParameter("titile");
		int m_index2 = Integer.parseInt(request.getParameter("index2"));
		int m_acticityID = Integer.parseInt(request.getParameter("id"));
		...
		activity.setActivityId(m_acticityID);
		
		int isSuccess = ActivityDao.updateActivity( activity);
		if (isSuccess > 0)
		{
